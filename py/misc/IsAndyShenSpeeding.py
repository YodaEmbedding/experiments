...
def get_point():
	response = urlopen('http://find.andyshen.ca/')
	html = response.read()

	# soup = BeautifulSoup(html, 'html.parser')
	# soup.find_all('div', class_='col-sm-3'))

	return Point(float(re.search(r"Latitude:\s+([\d\-\.]+)", str(html)).group(1)),
...
def distance(p1, p2):
	...
	a = math.sin(dLat / 2) * math.sin(dLat / 2) + \
		math.cos(math.radians(p1.latitude)) * math.cos(math.radians(p2.latitude)) * \
...
while True:
	...
	print('Latitude: {:<8}     Longitude: {:<8}    Distance (m): {:<8} Speed (km/h): {:<5}'.format(
