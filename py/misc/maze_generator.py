...
class Maze:
    ...
    def __str__(self):
        ...
        vert_rows = (
            [vert_str(row & Dir.N) for row in self.maze] +
            [vert_str(self.maze[-1] & Dir.S)])

        horz_rows = [horz_str(itertools.chain(row & Dir.W, [row[-1] & Dir.E]))
    ...
    def __getitem__(self, key):
        ...
        if all(x != d for x in (Dir.N, Dir.W, Dir.S, Dir.E)):
            raise ValueError
        return (
            self.maze[y, x] & d != 0 if d == Dir.N or d == Dir.W else
            self.maze[y, x + 1] & Dir.W != 0 if d == Dir.E else
            self.maze[y + 1, x] & Dir.N != 0 if d == Dir.S else
            None)
