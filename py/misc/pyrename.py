...
argsParser.add_argument('-c', '--noconfirmation', action="store_true", help="Suppress confirmation (if no conflicts)")
argsParser.add_argument('-d', '--subdirectories', action="store_true", help="Include subdirectories")
argsParser.add_argument('-f', '--find', type=str, action="store", help="Find regex")
argsParser.add_argument('-i', '--ignorecase', action="store_true", help="Ignore case")
argsParser.add_argument('-p', '--path', type=str, action="store", help="Directory (default is current directory)")
argsParser.add_argument('-r', '--replace', type=str, action="store", help="Replace regex (use \\1, \\2, ... for captures)")
argsParser.add_argument('-w', '--nowarnings', action="store_true", help="Suppress warnings")
args = argsParser.parse_args()

def getFilesRecursively(root: str):
	return [os.path.join(path, file) for path, subdirs, files in os.walk(root) for file in files]

def getFiles(path: str, ignoreSubdirectories: bool):
	return os.listdir(path) if ignoreSubdirectories else getFilesRecursively(path)
